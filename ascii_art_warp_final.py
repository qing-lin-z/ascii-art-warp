...
chcp 65001 > nul
title ASCII Art Warp - 命令行模式

echo ============================================
echo    ASCII Art Warp 命令行模式
echo ============================================
echo.

echo 请输入视频路径（支持拖拽文件到这里）：
set /p INPUT_PATH=

echo 请输入输出路径：
set /p OUTPUT_PATH=

echo 请输入输出宽度（字符数，默认80）：
set /p TARGET_W=
if "%%TARGET_W%%"=="" set TARGET_W=80

echo 请输入输出高度（字符数，默认45）：
set /p TARGET_H=
if "%%TARGET_H%%"=="" set TARGET_H=45

echo 是否使用彩色模式？（y/n，默认y）：
set /p USE_COLOR=
if "%%USE_COLOR%%"=="n" (
    set USE_COLOR_FLAG=--no-color
) else (
    set USE_COLOR_FLAG=--color
)

echo 是否压缩视频？（y/n，默认y）：
set /p COMPRESS=
if "%%COMPRESS%%"=="n" (
    set COMPRESS_FLAG=--no-compress
) else (
    set COMPRESS_FLAG=--compress
)

python "%~dp0ascii_art_warp_final.py" --cli ^
    --input "%%INPUT_PATH%%" ^
    --output "%%OUTPUT_PATH%%" ^
    --width %%TARGET_W%% ^
    --height %%TARGET_H%% ^
...
class AsciiArtApp:
    ...
    def create_widgets(self):
        ...
        tk.Button(fpar, text="选择", command=self.choose_bg_color).grid(row=r, column=2, sticky="w")
        r += 1
        ...
        tk.Button(self.root, text="导出命令行脚本", command=self.export_bat_gui,
                  bg="#d29922", font=("Arial", 10)).pack(pady=2)
    ...
    def export_bat_gui(self):
        """导出命令行批处理脚本"""
        import os
        script_dir = os.path.dirname(os.path.abspath(__file__))
        bat_path = os.path.join(script_dir, 'run_ascii.bat')
        try:
            with open(bat_path, 'w', encoding='utf-8') as f:
                f.write(generate_bat_script())
            messagebox.showinfo('成功', f'已导出:\n{bat_path}\n\n双击运行或在命令行执行')
        except Exception as e:
            messagebox.showerror('错误', f'导出失败:\n{e}')

    def choose_bg_color(self):
        result = colorchooser.askcolor(title="选择背景色")
        if result[0] is not None:
            c = result[0]
            self.bg_color_rgb = (int(c[0]), int(c[1]), int(c[2]))
            self.bg_color_display.config(bg='#%02x%02x%02x' % self.bg_color_rgb)
...
示例:
  ...
  # 导出批处理脚本
  python ascii_art_warp_final.py --export-bat
        '''
    )

    parser.add_argument('--cli', action='store_true', help='使用命令行模式（否则启动GUI）')
    parser.add_argument('--export-bat', action='store_true', help='导出Windows批处理脚本')
    ...
    # 导出 bat 脚本
    if args.export_bat:
        bat_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'run_ascii.bat')
        with open(bat_path, 'w', encoding='utf-8') as f:
            f.write(generate_bat_script())
        print(f'已导出批处理脚本: {bat_path}')
        print('双击运行或在命令行执行')
        sys.exit(0)
